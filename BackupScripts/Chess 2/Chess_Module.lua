local BoardSystem_Module = {}

local ServerStorage = game:GetService("ServerStorage")

local GameAssets = ServerStorage.GameAssets

BoardSystem_Module.CreateBoard = {}--Function map containing functions and data used to generate new boards to the game.

BoardSystem_Module.CreateBoard.DefaultBoardInfo = {} --Stores the default information used by the diferent board creating functions.


--============================================================================================================
-- CREATE BOARD: CHESS

BoardSystem_Module.CreateBoard.DefaultBoardInfo.Chess = { --Default parameters for a chess board.
    Rows = 8,
    Columns = 8,
...
--------------------------------------------------------------------------------------------------------
---Creates a chess board with the specified parameters.
---@param BoardInfo table Table containing the parameters of creation of the board.
---@return table Matrix containing all info regarding the board. Tiles organized as Row->Column
function BoardSystem_Module.CreateBoard.Chess(BoardInfo)

    --Verify BoardInfo parameters:
    local DefaultBoardInfo_Chess = BoardSystem_Module.CreateBoard.DefaultBoardInfo.Chess
    if BoardInfo ~= nil then

        for PIndex, Parameter in pairs(DefaultBoardInfo_Chess) do
            if BoardInfo[PIndex] == nil then
                BoardInfo[PIndex] = DefaultBoardInfo_Chess[PIndex]
            end
        end
        
    else
        BoardInfo = DefaultBoardInfo_Chess
    end
